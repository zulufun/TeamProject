...
class WiresharkApp:
    def __init__(self, master):
        self.master = master
        master.title("PacketCapture and Log Monitor App")
        #Add from 25 - 11 - 2024 -- Thêm tính năng logging
        self.notebook = ttk.Notebook(master)
        self.notebook.pack(fill='both', expand=True)
        # Tạo các frame cho các tab
        self.capture_frame = ttk.Frame(self.notebook)
        self.log_frame = ttk.Frame(self.notebook)
        self.notebook.add(self.capture_frame, text="Packet Capture")
        self.notebook.add(self.log_frame, text="Event Log Monitor")
        # Thêm các phần đã có vào capture_frame
        self.init_capture_tab(self.capture_frame)
        # Thêm phần theo dõi log vào log_frame
        self.init_log_tab(self.log_frame)
        # Các biến kiểm soát log
        self.log_running = False
        #########################3
        def init_capture_tab(self, frame):
            # Toàn bộ mã giao diện và logic bắt gói tin của bạn ở đây
            self.start_button = tk.Button(frame, text="Start Capture", command=self.start_capture)
            self.start_button.pack()
            # Thêm các phần còn lại từ mã gốc của bạn vào đây
