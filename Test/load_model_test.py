...
class Flow:
    ...
    def __init__(self, key, interface=None):
        ...
        self.label = "Loading..."  # Default label is "Loading..."
...
class PacketAnalyzer:
    ...
    def load_model(self):
        ...
        try:
            ...
            # After model is loaded, update all existing flows to be analyzed
            with self.flow_lock:
                for flow_key, flow in self.flows.items():
                    if flow.label == "Loading...":
                        try:
                            ...
                            print(f"Error predicting flow label during initial analysis: {e}")
                            flow.label = "Error"
                            self.model_queue.put(flow)
            ...
            self.model_loaded = False
            print("Model loading failed, flows will remain in Loading state")
        ...
        while self.is_capturing:
            try:
                ...
                with self.flow_lock:
                    ...
                    # Try to predict label if model is loaded and flow still has Loading label
...
class NetworkAnalyzerGUI:
    ...
    def update_table(self):
        ...
        try:
            # Configure tag colors - do this once before any item updates
            ...
            self.tree.tag_configure('loading', background='#ffffcc')  # Light yellow for loading
            ...
            if updated_flow:
                ...
                for item_id in self.tree.get_children():
                    ...
                    if (item['values'][2] == updated_flow.dst_port and
                        ...
                        elif updated_flow.label.lower() == "benign":
                            self.tree.item(item_id, values=values, tags=('benign',))
                        else:  # For "Loading..." or errors
                            self.tree.item(item_id, values=values, tags=('loading',))
            ...
            if self.analyzer.is_capturing:
                ...
                for flow in flows:
                    ...
                    # Only assign a tag if it matches one of our defined conditions
                    if flow['Label'].lower() == 'benign':
                        tag = 'benign'
                    elif flow['Label'].lower() == 'malicious':
                        tag = 'malicious'
                    elif flow['Label'] == 'Loading...':
                        tag = 'loading'
                    else:
                        tag = ''  # No tag for other cases
